...
#### ====== Read recipe vecto from json file  ===============================
    with open('./spark_recipe_vector.json', 'r', encoding='utf-8') as file:
        txt = file.readlines()
    ...
    wordvec = np.zeros([1, 50], dtype=float)
    for n, item in enumerate(list_history):
        for check in target_list:
            if item == check['title']:
                wordvec = wordvec + check['result']['values']
                break
...
def cosine_distance_userhistory(uservec, target_list, num):
    ...
    for item in target_list:
        ...
        b = item['result']['values']
        cos_sim = dot(a, b) / (norm(a) * norm(b))
        inser_value = (cos_sim, url, img_url)
...
def main(list_ingredient):
    ...
    callback = cosine_distance_userhistory(my_prefer, target_list, 100)
    print(callback)
